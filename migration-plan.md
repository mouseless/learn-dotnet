...
To use these new collection expressions with custom collections, your custom
collection class need to implement `IEnumerable<T>` for `GetEnumerator()`
method. With the enumerator you can use your custom collection as a
`spread_element` when initializing another collection.

```csharp
public class CustomCollection : IEnumerable<SomeClass>
{
    internal readonly List<SomeClass> _someClassList = [];

    public IEnumerator<SomeClass> GetEnumerator()
    {
        return ((IEnumerable<SomeClass>)_someClassList).GetEnumerator();
    }

    IEnumerator IEnumerable.GetEnumerator()
    {
        return ((IEnumerable)_someClassList).GetEnumerator();
    }
}
...
provide an add method. For immutable collections this add method will be
pointless thus only mutable custom collections are eligible.

```csharp
public class CustomCollection : IEnumerable<SomeClass>
{
  ...
  public void Add(SomeClass someClass) => _someClassList.Add(someClass);
}
...
collection and use the new collection expression features. However for your
custom collection to be used as `spread_element` you still need to implement
`IEnumerable<T>`. The first parameter of the attribute is the type of your
builder, and the second parameter is the method from the builder to invoke.

```csharp
[CollectionBuilder(typeof(CustomCollectionBuilder), "Create")]
public class CustomCollection(params SomeClass[] someclasses) : IEnumerable<SomeClass>
{
    public IEnumerator<SomeClass> GetEnumerator()
    {
        return ((IEnumerable<SomeClass>)someclasses).GetEnumerator();
    }

    IEnumerator IEnumerable.GetEnumerator()
    {
        return someclasses.GetEnumerator();
    }
}

public static class CustomCollectionBuilder
{
  public static CustomCollection Create(ReadOnlySpan<SomeClass> someClasses) =>
    new CustomCollection(someClasses.ToArray());
}
```

Using the collection builder or without, you can use your custom collections as
shown below.

```csharp
 var someClass = new SomeClass();
 var someClass2 = new SomeClass();
 var someClass3 = new SomeClass();
 var someClass4 = new SomeClass();

 CustomCollection customCollection = [someClass, someClass2, someClass3, someClass4];
 List<SomeClass> someClassList = [.. customCollection];

 CustomCollection anotherCustomCollection = [.. customCollection, .. someClassList, someClass, someClass2];
```
